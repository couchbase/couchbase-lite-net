...
. Install Couchbase Lite from Nuget (or if you are interested in a prerelease build, install from http://mobile.nuget.couchbase.com/nuget/Developer).
+
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/[`.NET SDK API References`]
...
[source,csharp]
----
include::{examplesdir}/Program.cs[tags=query-collection-operator-contains]
----
...
[source,csharp]
----
include::{examplesdir}/Program.cs[tags=query-collection-operator-in]
----
...
`MutableDocument`, `MutableDictionary` and `MutableArray`.
