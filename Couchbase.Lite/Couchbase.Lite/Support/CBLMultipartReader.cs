...
namespace Couchbase.Support
{
	...
	{
		private enum CBLMultipartReaderState
		{
			kUninitialized,
			kAtStart,
			kInPrologue,
		...
		private CBLMultipartReader.CBLMultipartReaderState state;
		...
		private CBLMultipartReaderDelegate delegate_;

		public IDictionary<string, string> headers;

		public CBLMultipartReader(string contentType, CBLMultipartReaderDelegate delegate_
			)
		{
			this.contentType = contentType;
			this.delegate_ = delegate_;
			this.buffer = new ByteArrayBuffer(1024);
			this.state = CBLMultipartReader.CBLMultipartReaderState.kAtStart;
			ParseContentType();
		}
		...
		{
			return state == CBLMultipartReader.CBLMultipartReaderState.kAtEnd;
		}
		...
		public virtual void AppendData(byte[] data)
		{
			...
			CBLMultipartReader.CBLMultipartReaderState nextState;
			do
			{
				nextState = CBLMultipartReader.CBLMultipartReaderState.kUninitialized;
				int bufLen = buffer.Length();
				switch (state)
				{
					case CBLMultipartReader.CBLMultipartReaderState.kAtStart:
					{
						...
						if (bufLen >= boundaryWithoutLeadingCRLF.Length)
						{
								...
								.Length))
							{
								...
								nextState = CBLMultipartReader.CBLMultipartReaderState.kInHeaders;
							}
							else
							{
								nextState = CBLMultipartReader.CBLMultipartReaderState.kInPrologue;
							}
						}
						break;
					}

					case CBLMultipartReader.CBLMultipartReaderState.kInPrologue:
					case CBLMultipartReader.CBLMultipartReaderState.kInBody:
					{
						...
						if (r.GetLength() > 0)
						{
							if (state == CBLMultipartReader.CBLMultipartReaderState.kInBody)
							{
					...
					case CBLMultipartReader.CBLMultipartReaderState.kInHeaders:
					{
						// First check for the end-of-message string ("--" after separator):
						if (bufLen >= 2 && Memcmp(buffer.ToByteArray(), EomBytes(), 2))
						{
							state = CBLMultipartReader.CBLMultipartReaderState.kAtEnd;
							Close();
							return;
						}
						...
						if (r.GetLength() > 0)
						{
							...
							nextState = CBLMultipartReader.CBLMultipartReaderState.kInBody;
						}
						break;
					}
				...
				if (nextState != CBLMultipartReader.CBLMultipartReaderState.kUninitialized)
				{
					state = nextState;
				}
			}
			while (nextState != CBLMultipartReader.CBLMultipartReaderState.kUninitialized && 
				buffer.Length() > 0);
		}
