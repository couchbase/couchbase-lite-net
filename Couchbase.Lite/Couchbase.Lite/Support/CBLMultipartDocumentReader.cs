...
{
	public class CBLMultipartDocumentReader : CBLMultipartReaderDelegate
	{
		...
		private IDictionary<string, CBLBlobStoreWriter> attachmentsByName;

		private IDictionary<string, CBLBlobStoreWriter> attachmentsByMd5Digest;
		...
		public virtual void ParseJsonBuffer()
		{
			try
			{
				document = CBLServer.GetObjectMapper().ReadValue<IDictionary>(jsonBuffer.ToByteArray
					());
			}
			catch (IOException e)
			{
		...
		public virtual void SetContentType(string contentType)
		{
			...
			attachmentsByName = new Dictionary<string, CBLBlobStoreWriter>();
			attachmentsByMd5Digest = new Dictionary<string, CBLBlobStoreWriter>();
		}
