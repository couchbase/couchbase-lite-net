...
namespace Couchbase.Auth
{
	public class CBLPersonaAuthorizer : CBLAuthorizer
	{
			...
			)
		{
			...
			if (exp.Before(now))
			{
				Log.W(CBLDatabase.Tag, string.Format("%s assertion for %s expired: %s", this.GetType
		...
		public virtual string AssertionForSite(Uri site)
		{
			...
			if (assertion == null)
			{
				Log.W(CBLDatabase.Tag, string.Format("%s %s no assertion found for: %s", this.GetType
		...
		{
			lock (typeof(CBLPersonaAuthorizer))
			{
				string email;
				string origin;
				IDictionary<string, object> result = ParseAssertion(assertion);
		...
		public static IDictionary<string, object> ParseAssertion(string assertion)
		{
			...
			try
			{
				...
				Log.D(CBLDatabase.Tag, "CBLPersonaAuthorizer exp: " + expObject + " class: " + expObject
					.GetType());
