...
using Com.Couchbase.Lite.Router;
using Com.Couchbase.Lite.Util;
using Org.Apache.Http.Client;
using Sharpen;

namespace Com.Couchbase.Lite.Router
{
	public class Router : Database.ChangeListener
	{
		...
		public virtual void Start()
		{
			...
			try
			{
				MethodInfo m = typeof(Com.Couchbase.Lite.Router.Router).GetMethod(message, typeof(
			...
			catch (NoSuchMethodException)
			{
				try
				{
					...
					MethodInfo m = typeof(Com.Couchbase.Lite.Router.Router).GetMethod("do_UNKNOWN", typeof(
						Database), typeof(string), typeof(string));
					status_1 = (Status)m.Invoke(this, db, docID, attachmentName);
				}
				catch (Exception e)
