...
namespace Com.Couchbase.Lite
{
	...
	public class Database
	{
		...
		private IDictionary<string, ValidationBlock> validations;
		...
		{
			Com.Couchbase.Lite.Database.filterCompiler = filterCompiler;
		}
		...
		public virtual int GetDocumentCount()
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting document count", e);
			}
			finally
			{
				if (cursor != null)
		...
		public virtual long GetLastSequenceNumber()
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting last sequence", e);
			}
			finally
			{
				if (cursor != null)
		...
		public virtual Status Compact()
		{
			...
			try
			{
				Log.V(Com.Couchbase.Lite.Database.Tag, "Deleting JSON of old revisions...");
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error compacting", e);
				return new Status(Status.InternalServerError);
			}
			Log.V(Com.Couchbase.Lite.Database.Tag, "Deleting old attachments...");
			Status result = GarbageCollectAttachments();
			Log.V(Com.Couchbase.Lite.Database.Tag, "Vacuuming SQLite sqliteDb...");
			try
			{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error vacuuming sqliteDb", e);
				return new Status(Status.InternalServerError);
			}
			return result;
		}
		...
		/// </remarks>
		[InterfaceAudience.Public]
		public virtual ValidationBlock GetValidation(string name)
		{
		...
		/// </remarks>
		[InterfaceAudience.Public]
		public virtual void SetValidation(string name, ValidationBlock validationBlock)
		{
			if (validations == null)
			{
				validations = new Dictionary<string, ValidationBlock>();
			}
		...
		public virtual ReplicationFilter GetFilter(string filterName)
		{
			...
			if (result == null)
			{
				...
				if (filter == null)
				{
					Log.W(Com.Couchbase.Lite.Database.Tag, string.Format("Filter %s failed to compile"
						, filterName));
					return null;
				}
		...
		public virtual bool RunInTransaction(TransactionTask databaseFunction)
		{
			...
			{
				shouldCommit = false;
				Log.E(Com.Couchbase.Lite.Database.Tag, e.ToString(), e);
		...
		internal virtual Future RunAsync(AsyncTask function)
		{
			return GetManager().RunAsync(new _Runnable_585(this, function));
		}

		private sealed class _Runnable_585 : Runnable
		{
			public _Runnable_585(Database _enclosing, AsyncTask function)
			{
		...
		/// <returns></returns>
		[InterfaceAudience.Public]
		public virtual Replication GetPushReplication(Uri remote)
		{
		...
		/// <returns></returns>
		[InterfaceAudience.Public]
		public virtual Replication GetPullReplication(Uri remote)
		{
		...
		/// 	</returns>
		[InterfaceAudience.Public]
		public virtual IList<Replication> GetReplications(Uri remote)
		{
			Replication pull;
			Replication push;
			if (remote != null)
			{
				...
				AList<Replication> result = new AList<Replication>();
				result.AddItem(pull);
				result.AddItem(push);
				return result;
			}
		...
		/// 	</remarks>
		public virtual Document GetCachedDocument(string documentID)
		{
		...
		public static Com.Couchbase.Lite.Database CreateEmptyDBAtPath(string path, Manager
			 manager)
		{
			...
			Com.Couchbase.Lite.Database result = new Com.Couchbase.Lite.Database(path, manager
				);
		...
		public virtual bool Open()
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error turning on foreign keys");
				return false;
			}
			...
			if (dbVersion >= 100)
			{
				Log.W(Com.Couchbase.Lite.Database.Tag, "Database: Database version (" + dbVersion
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Could not initialize attachment store", e
					);
				database.Close();
				return false;
			}
		...
		public virtual bool BeginTransaction()
		{
			try
			{
				...
				++transactionLevel;
				Log.I(Com.Couchbase.Lite.Database.TagSql, Sharpen.Thread.CurrentThread().GetName(
					) + " Begin transaction (level " + Sharpen.Extensions.ToString(transactionLevel)
					 + ")");
			}
			catch (SQLException e)
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, Sharpen.Thread.CurrentThread().GetName() +
			...
			)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error convert extra JSON to bytes", e);
				return null;
			}
			...
			> contentOptions)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error serializing properties to JSON", e);
			}
			return docProperties;
		}
			...
			<Database.TDContentOptions> contentOptions)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting document with id and rev", 
					e);
			}
			finally
			{
			...
			> contentOptions)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error loading revision body", e);
		...
		public virtual long GetDocNumericID(string docId)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting doc numeric id", e);
			}
			finally
			{
				if (cursor != null)
			...
			, bool onlyCurrent)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting all revisions of document", 
					e);
				return null;
			}
			finally
		...
		public virtual IList<RevisionInternal> GetRevisionHistory(RevisionInternal rev)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting revision history", e);
				return null;
			}
			finally
			{
			 ...
			 filter)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error looking for changes", e);
			}
			finally
			{
				if (cursor != null)
			...
			, IList<long> outLastSequence)
		{
			...
			if (viewName != null && viewName.Length > 0)
			{
				...
				else
				{
					if (options.GetStale() == Query.IndexUpdateMode.After && lastSequence < GetLastSequenceNumber
						())
					{
						new Sharpen.Thread(new _Runnable_1580(view)).Start();
					}
				}
			...
			Log.D(Com.Couchbase.Lite.Database.Tag, string.Format("Query view %s completed in %d milliseconds"
				, viewName, delta));
			return rows;
		}

		private sealed class _Runnable_1580 : Runnable
		{
			public _Runnable_1580(View view)
			{
				this.view = view;
			}
			...
			public void Run()
			{
				...
				{
					Log.E(Com.Couchbase.Lite.Database.Tag, "Error updating view index on background thread"
						, e);
				}
			}
		...
		public virtual IList<View> GetAllViews()
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting all views", e);
			}
			finally
			{
				if (cursor != null)
		...
		public virtual Status DeleteViewNamed(string name)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error deleting view", e);
			}
			return result;
		}
		...
		public virtual IDictionary<string, object> GetAllDocs(QueryOptions options)
		{
			...
			try
			{
				...
				while (!cursor.IsAfterLast())
				{
					...
					bool deleted = options.IsIncludeDeletedDocs() && cursor.GetInt(GetDeletedColumnIndex
						(options)) > 0;
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting all docs", e);
			...
			, IList<bool> outIsConflict)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error", e);
			 ...
			 name, string contentType, int revpos, BlobKey key)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error inserting attachment", e);
				throw new CouchbaseLiteException(Status.InternalServerError);
			}
		}
			...
			, long toSeq)
		{
			...
			try
			{
				...
				if (rowsUpdated == 0)
				{
					...
					Log.W(Com.Couchbase.Lite.Database.Tag, "Can't find inherited attachment " + name 
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error copying attachment", e);
				throw new CouchbaseLiteException(Status.InternalServerError);
			}
			finally
			{
			...
			)
		{
			...
			try
			{
				...
				{
					Log.E(Com.Couchbase.Lite.Database.Tag, "Failed to load attachment");
					throw new CouchbaseLiteException(Status.InternalServerError);
				}
				else
				{
			...
			(long sequence, EnumSet<Database.TDContentOptions> contentOptions)
		{
				...
				while (!cursor.IsAfterLast())
				{
					...
					if (contentOptions.Contains(Database.TDContentOptions.TDIncludeAttachments))
					{
						if (contentOptions.Contains(Database.TDContentOptions.TDBigAttachmentsFollow) && 
							length >= Com.Couchbase.Lite.Database.kBigAttachmentLength)
						{
							dataSuppressed = true;
						}
						else
						{
							...
							{
								// <-- very expensive
								Log.W(Com.Couchbase.Lite.Database.Tag, "Error loading attachment");
							}
						}
					}
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting attachments for sequence", 
					e);
				return null;
			}
			finally
		...
		public virtual void StubOutAttachmentsIn(RevisionInternal rev, int minRevPos)
		{
			...
			foreach (string name in attachments.Keys)
			{
				...
				if (revPos > 0 && revPos < minRevPos && (stub == null))
				{
					...
					Log.D(Com.Couchbase.Lite.Database.Tag, "Stubbed out attachment" + rev + " " + name
			...
			foreach (string name in revAttachments.Keys)
			{
				...
				if (attachment != null)
				{
					...
					else
					{
						...
						{
							Log.W(Com.Couchbase.Lite.Database.Tag, string.Format("Attachment %s %s has unexpected revpos %s, setting to %s"
		...
		public virtual Status GarbageCollectAttachments()
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error deleting attachments", e);
			}
			...
			try
			{
				...
				Log.V(Com.Couchbase.Lite.Database.Tag, "Deleted " + numDeleted + " attachments");
				return new Status(Status.Ok);
			}
			catch (SQLException e)
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error finding attachment keys in use", e);
				return new Status(Status.InternalServerError);
			}
			finally
			{
		...
		public virtual long InsertDocumentID(string docId)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error inserting document id", e);
			}
			return rowId;
		}
		...
		public virtual byte[] EncodeDocumentJSON(RevisionInternal rev)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error serializing " + rev + " to JSON", e
					);
			}
			return json;
		}

		public virtual void NotifyChange(RevisionInternal rev, Uri source)
		{
			...
			parentSequence, bool current, byte[] data)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error inserting revision", e);
			}
			return rowId;
		}
			...
			, bool allowConflict, Status resultStatus)
		{
			...
			try
			{
				...
				else
				{
					...
					else
					{
						...
						docId = Com.Couchbase.Lite.Database.GenerateDocumentId();
						docNumericID = InsertDocumentID(docId);
						if (docNumericID <= 0)
						{
							return null;
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error putting revision", e1);
				return null;
			}
			finally
			{
		...
		public virtual bool FindMissingRevisions(RevisionList touchRevs)
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error finding missing revisions", e);
				return false;
			}
			finally
			{
		...
		{
			IDictionary<string, object> result = new Dictionary<string, object>();
			RunInTransaction(new _TransactionTask_3241(this, docsToRevs, result));
		...
		private sealed class _TransactionTask_3241 : TransactionTask
		{
			public _TransactionTask_3241(Database _enclosing, IDictionary<string, IList<string
					...
					else
					{
						...
						else
						{
							if (revIDs.Contains("*"))
							{
								...
								{
									Log.E(Com.Couchbase.Lite.Database.Tag, "Error deleting revisions", e);
									return false;
								}
								revsPurged = new AList<string>();
								revsPurged.AddItem("*");
							...
							else
							{
								...
								try
								{
									...
									if (!cursor.MoveToNext())
									{
										Log.W(Com.Couchbase.Lite.Database.Tag, "No results for query: " + queryString);
										return false;
									}
									...
									Log.I(Com.Couchbase.Lite.Database.Tag, string.Format("Purging doc '%s' revs (%s); asked for (%s)"
									...
									if (seqsToPurge.Count > 0)
									{
										...
										{
											Log.E(Com.Couchbase.Lite.Database.Tag, "Error deleting revisions via: " + sql, e);
											return false;
										}
									}
								...
								{
									Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting revisions", e);
									return false;
								}
								finally
								{
		...
		protected internal virtual bool ReplaceUUIDs()
		{
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error updating UUIDs", e);
				return false;
			}
		...
		public virtual RevisionInternal GetLocalDocument(string docID, string revID)
		{
			...
			try
			{
				...
				if (cursor.MoveToNext())
				{
					...
					{
						Log.W(Com.Couchbase.Lite.Database.Tag, "Error parsing local doc JSON", e);
						return null;
					}
				}
				return result;
			...
			{
				Log.E(Com.Couchbase.Lite.Database.Tag, "Error getting local document", e);
				return null;
			}
			finally
			{
	...
	internal class TDValidationContextImpl : ValidationContext
	{
		...
		/// <exception cref="Com.Couchbase.Lite.CouchbaseLiteException"></exception>
		public virtual RevisionInternal GetCurrentRevision()
		{
		...
		public virtual Status GetErrorType()
		{
			return errorType;
		}

		public virtual void SetErrorType(Status status)
		{
