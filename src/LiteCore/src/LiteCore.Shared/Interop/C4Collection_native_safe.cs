...
internal sealed unsafe class C4CollectionWrapper : NativeWrapper
{
    public delegate T NativeCallback<out T>(C4Collection* c);
    ...
    protected override void Dispose(bool disposing) => Native.c4coll_release(RawCollection);
}
...
internal static unsafe partial class NativeSafe
{
    ...
    public static C4CollectionWrapper? c4db_getDefaultCollection(C4DatabaseWrapper database, C4Error* outError)
    {
        ...
        return rawColl == null ? null : new C4CollectionWrapper(rawColl, database);

    }

    public static bool c4coll_isValid(C4CollectionWrapper collection) => Native.c4coll_isValid(collection.RawCollection);

    // Database Exclusive Methods

    public static bool c4db_hasScope(C4DatabaseWrapper database, string? name) => 
        database.UseSafe(db => Native.c4db_hasScope(db, name));
    ...
    public static C4CollectionWrapper? c4db_getCollection(C4DatabaseWrapper database, C4CollectionSpec spec, C4Error* outError)
    {
        ...
        return nativeCollection == null ? null : new C4CollectionWrapper(nativeCollection, database);
    ...
    public static bool c4db_deleteCollection(C4DatabaseWrapper database, C4CollectionSpec spec, C4Error* outError) => 
        database.UseSafe(db => Native.c4db_deleteCollection(db, spec, outError));

    public static FLMutableArray* c4db_collectionNames(C4DatabaseWrapper database, string? inScope, C4Error* outError) => 
        (FLMutableArray*)database.UseSafe(db => (IntPtr)Native.c4db_collectionNames(db, inScope, outError));

    public static FLMutableArray* c4db_scopeNames(C4DatabaseWrapper database, C4Error* outError) => 
        (FLMutableArray*)database.UseSafe(db => (IntPtr)Native.c4db_scopeNames(db, outError));

    public static ulong c4coll_getDocumentCount(C4CollectionWrapper collection) => collection.UseSafe(Native.c4coll_getDocumentCount);
