...
internal unsafe sealed class C4ReplicatorWrapper : NativeWrapper
{
    public delegate void NativeCallback(C4Replicator* r);
    public delegate T NativeCallback<T>(C4Replicator* r);
    ...
    {
        if (!(obj is C4ReplicatorWrapper other)) {
            return false;
        }
    ...
    {
        return (int)RawReplicator;
    }

    protected override void Dispose(bool disposing)
    {
        Native.c4repl_free(RawReplicator);
    }
}
...
internal static unsafe partial class NativeSafe
{
    ...
    {
        return NativeRaw.c4address_fromURL(url, address, dbName);
    }

    public static void c4repl_start(C4ReplicatorWrapper repl, bool reset)
    {
        Native.c4repl_start(repl.RawReplicator, reset);
    }

    public static void c4repl_stop(C4ReplicatorWrapper repl)
    {
        Native.c4repl_stop(repl.RawReplicator);
    }
    ...
    {
        return Native.c4repl_getStatus(repl.RawReplicator);
    }
    ...
    public static C4ReplicatorWrapper? c4repl_new(C4DatabaseWrapper database, C4Address remoteAddress, string? remoteDatabaseName, C4ReplicatorParameters @params, string? logPrefix, C4Error* outError)
    {
        ...
        return new C4ReplicatorWrapper(rawRepl);
    }
    ...
    {
        return database.UseSafe(db => Native.c4db_setCookie(db, setCookieHeader, fromHost, fromPath, acceptParentDomain, outError));
    }

    public static string? c4db_getCookies(C4DatabaseWrapper database, C4Address request, C4Error* outError)
    {
        return database.UseSafe(db => Native.c4db_getCookies(db, request, outError));
    }
    ...
    {
        return repl.UseSafe(r => Native.c4repl_setProgressLevel(r, level, outError));
    }
}