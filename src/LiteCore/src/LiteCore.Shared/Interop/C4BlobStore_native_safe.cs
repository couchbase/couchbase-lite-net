...
internal sealed unsafe class C4ReadStreamWrapper : NativeWrapper
{
    public delegate T NativeCallback<T>(C4ReadStream* s);

    public C4ReadStream* RawStream => (C4ReadStream*)_nativeInstance;

    public C4ReadStreamWrapper(C4ReadStream* readStream)
        : base((IntPtr)readStream)
    {
...
internal sealed unsafe class C4WriteStreamWrapper : NativeWrapper
{
    public delegate T NativeCallback<T>(C4WriteStream* s);

    public C4WriteStream* RawStream => (C4WriteStream*)_nativeInstance;

    public C4WriteStreamWrapper(C4WriteStream* readStream)
        : base((IntPtr)readStream)
    {
...
internal static unsafe partial class NativeSafe
{
    ...
    public static bool c4blob_keyFromString(string? keyString, C4BlobKey* key)
    {
        return Native.c4blob_keyFromString(keyString, key);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static string? c4blob_keyToString(C4BlobKey key)
    {
        return Native.c4blob_keyToString(key);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static long c4blob_getSize(C4BlobStore* store, C4BlobKey key)
    {
        return Native.c4blob_getSize(store, key);
    }
    ...
    {
        return Native.c4blob_create(store, contents, expectedKey, outKey, outError);
    }
    ...
    public static C4WriteStreamWrapper? c4blob_openWriteStream(C4BlobStore* store, C4Error* outError)
    {
        ...
        return new C4WriteStreamWrapper(rawStream);
    }
    ...
    {
        return (C4BlobStore*)database.UseSafe(db => (IntPtr)Native.c4db_getBlobStore(db, outError));
    }
