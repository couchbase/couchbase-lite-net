...
internal sealed unsafe class LiveQuerier : IDisposable
{
    ...
    private readonly QueryBase _queryBase;
    ...
    internal LiveQuerier CreateLiveQuerier(C4QueryWrapper c4Query)
    {
    ...
    private static void QueryObserverCallback(C4QueryObserver* obs, C4Query* query, void* context)
    {
        ...
        obj.QueryObserverCalled();
    }

    private void QueryObserverCalled()
    {
        _queryBase.DispatchQueue.DispatchSync(() =>
        {
            if(_disposed) {
    ...
    private QueryResultSet? GetResults()
    {
        ...
        var newEnum = LiteCoreBridge.CheckTyped(err => 
            NativeSafe.c4queryobs_getEnumerator(_queryObserver, true, err));

        return newEnum != null 
            ? new QueryResultSet(_queryBase, _queryBase.ThreadSafety, newEnum, _queryBase.ColumnNames) 
            : null;
