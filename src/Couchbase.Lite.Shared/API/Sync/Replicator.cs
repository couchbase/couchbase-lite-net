    ...
    IDocumentReplicatedObservable
{
    ...
    private readonly
    #endif
        CancellationTokenSource _conflictCancelSource = new CancellationTokenSource();
    ...
    /// <summary>
    /// Checks whether a document with the given ID in the given collection is pending to push or not. 
    ...
    /// <returns>A bool which represents whether the document with the corresponding ID has one or more pending revisions.  
    ...
    public bool IsDocumentPending(string documentID, Collection collection)
    {
        ...
        using var c4CollName = new C4String(collection.Name);
        using var c4ScopeName = new C4String(collection.Scope.Name);
        ...
        return LiteCoreBridge.Check(err => 
            _repl != null && NativeSafe.c4repl_isDocumentPending(_repl, documentID, collectionSpec, err));
    }
    ...
    public IImmutableSet<string> GetPendingDocumentIDs(Collection collection)
    {
        ...
        using (var c4ScopeName = new C4String(collection.Scope.Name)) {
            ...
            var pendingDocIds = LiteCoreBridge.Check(err =>
            {
                if(_repl == null) {
                    return null;
                }
            ...
            if (pendingDocIds != null) {
                ...
                for (var i = 0; i < cnt; i++) {
                    ...
                    var nextId = CBDebug.MustNotBeNull(WriteLog.To.Sync, Tag, "flv", Native.FLValue_AsString(flv));
                    result.Add(nextId);
                }
        ...
        _pendingDocIds = result.ToImmutableHashSet();
    ...
    private void OnDocEndedWithConflict(List<ReplicatedDocument> replications)
    {
        ...
        foreach (var replicatedDocument in replications) {
            var replication = replicatedDocument;
            ...
            {
                try {
                    var coll = Config.Collections.First(x => 
                        x.Name == replication.CollectionName && x.Scope.Name == replication.ScopeName);
                ...
                _documentEndedUpdate.Fire(this, new DocumentReplicationEventArgs([replication], false));
    ...
    private C4Error SetupC4Replicator()
    {
        ...
        var dbNameStr = remoteUrl?.Segments?.Last().TrimEnd('/');
        using var c4DbName = new C4String(dbNameStr);
        using var c4RemoteUrl = new C4String(remoteUrl?.AbsoluteUri);
        
        {
            var dn = c4DbName.AsFLSlice();
            C4Address localAddr;

            // Note: Don't use Native.c4address_fromURL, remoteUrlStr_ MUST stay alive for this entire method
            var addrFromUrl = NativeSafe.c4address_fromURL(c4RemoteUrl.AsFLSlice(), &localAddr, &dn);
            addr = localAddr;
            ...
            DispatchQueue.DispatchSync(() =>
            {
                ...
                using (var _ = _nativeParams.Pinned()) {
#if COUCHBASE_ENTERPRISE
                    // Both c4 databases are assured not null by CheckOpenLocked previously
                    if (otherDB != null) {
