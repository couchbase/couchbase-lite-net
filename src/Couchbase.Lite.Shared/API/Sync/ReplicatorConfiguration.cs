...
public sealed partial record ReplicatorConfiguration
{
    ...
    private readonly MutableState _mutableState;
    ...
    public required IEndpoint Target
    {
        get => _target;
        init {
            var castTarget = Misc.TryCast<IEndpoint, IEndpointInternal>(value);
            castTarget.Visit(_mutableState);
            _target = value;
        }
    }
    ...
    /// </summary>
    public required IImmutableList<CollectionConfiguration> CollectionConfigurations { get; init; }
    ...
    {
        get {
            Debug.Assert(CollectionConfigurations.Count != 0);
            return CollectionConfigurations[0].Collection.Database;
        }
    }

    internal Uri? RemoteUrl => _mutableState.RemoteUrl;

    internal Database? OtherDB => _mutableState.OtherDB;

    internal C4SocketFactory SocketFactory => _mutableState.SocketFactory;
        ...
        IEndpoint target)
    {
        ...
        CollectionConfigurations = collections.ToImmutableArray();
        if (CollectionConfigurations.Count == 0) {
        ...
        foreach (var c in CollectionConfigurations.Where(c => !ReferenceEquals(c.Collection.Database, DatabaseInternal))) {
    ...
    // This class holds the state of the configuration that is changed by Target.Visit()
    // The mutable state is kept separate to keep the base class read only.  When copying
    // the configuration, the mutable state will be copied as-is unless the Target is 
    // changed, in which case it will be mutated once again as appropriate.
    internal sealed class MutableState(ReplicatorConfiguration parent)
    {
        private C4SocketFactory _socketFactory;

        public Uri? RemoteUrl;
        
        public Database? OtherDB;

        public string? RemoteDBUniqueID
        {
            set => parent.Options.RemoteDBUniqueID = value;
        }
