...
public sealed class MutableDocument : Document, IMutableDictionary
{
    ...
    /// <inheritdoc cref="IMutableDictionaryFragment.this" />
    ...
    private static object? MutableCopy(object? original)
    {
        ...
        {
            DictionaryObject dict => dict.ToMutable(),
            ArrayObject arr => arr.ToMutable(),
            IList list => new List<object>(list.Cast<object>()),
            IDictionary<string, object> netDict => new Dictionary<string, object>(netDict),
            _ => original
        };
    }
    ...
    /// <inheritdoc />
    public new MutableArrayObject? GetArray(string key) => Dict?.GetArray(key);

    /// <inheritdoc />
    public new MutableDictionaryObject? GetDictionary(string key) => Dict?.GetDictionary(key);
