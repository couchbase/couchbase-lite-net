...
{
    internal bool HasChanges => Dict.IsMutated;
    ...
    /// <summary>
    /// Creates a dictionary given the initial set of keys and values
    /// from a json dictionary string
    /// </summary>
    /// <param name="json">
    /// The json dictionary string to copy the keys and values from 
    /// </param>
    public MutableDictionaryObject(string json) => SetJSON(json);

    internal MutableDictionaryObject(MDict dict, bool isMutable) => Dict.InitAsCopyOf(dict, isMutable);
    ...
    private void SetValueInternal(string key, object? value)
    {
        ...
        var oldValue = Dict.Get(key);
        value = DataOps.ToCouchbaseObject(value);
        if (!DataOps.ValueWouldChange(value, oldValue, Dict)) {
            return;
        }
        
        Dict.Set(key, new MValue(value));
        KeysChanged();
    }

    /// <inheritdoc />
    public new MutableArrayObject? GetArray(string key) => base.GetArray(key) as MutableArrayObject;

    /// <inheritdoc />
    public new MutableDictionaryObject? GetDictionary(string key) => base.GetDictionary(key) as MutableDictionaryObject;
    ...
    public IMutableDictionary SetData(IDictionary<string, object?> dictionary)
    {
        ...
        foreach (var item in dictionary) {
            Dict.Set(item.Key, new MValue(item.Value));
        }
