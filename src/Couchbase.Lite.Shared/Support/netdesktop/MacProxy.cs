...
internal sealed class MacProxy : IProxy
{
    ...
    public unsafe Task<WebProxy?> CreateProxyAsync(Uri destination)
    {
        ...
        var proxySettings = MacProxy.CopySystemProxySettings(cFNetworkHandle);
    ...
    {
        return LoadLibrary(File.Exists(CFNetworkLibraryOld) 
            ? CFNetworkLibraryOld : CFNetworkLibrary);
    }
    ...
    private static IntPtr CopySystemProxySettings(IntPtr cFNetworkHandle) => 
        GetDelegate<CFNetworkCopySystemProxySettings>(cFNetworkHandle)();

    private static IntPtr CopyProxiesForURL(IntPtr cFNetworkHandle, IntPtr url, IntPtr proxySettings) => 
        GetDelegate<CFNetworkCopyProxiesForURL>(cFNetworkHandle)(url, proxySettings);
