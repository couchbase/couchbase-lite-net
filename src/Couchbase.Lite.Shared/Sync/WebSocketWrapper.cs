...
internal sealed class TlsCertificateReceivedEventArgs(X509Certificate2 peerCert) : EventArgs
{
    public X509Certificate2 PeerCertificate { get; } = new(peerCert);
}
...
// safety.
//
// Example: When it is time to write, LiteCore will call back into the C# callback, and
...
internal sealed class WebSocketWrapper
{
    ...
    // This starts the flow of data, and it is quite an intense multistep process
    ...
    private TcpClient CreateClientFromNetworkInterface()
    {
        try {
            ...
            if (IPAddress.TryParse(_options.NetworkInterface, out var addr)) {
                var localEndPoint = new IPEndPoint(addr, 0);
                return new TcpClient(localEndPoint);
            }

            //Get Network Interface
            var ni = NetworkInterface.GetAllNetworkInterfaces().FirstOrDefault(x => x.Name == _options.NetworkInterface
            ...
            var isRemoteHostIP = IPAddress.TryParse(_logic.UrlRequest.Host, out var hostAddr);
            if (isRemoteHostIP) {
    ...
    {
        if (!parser.Headers.TryGetValue(key, out var value)) {
            return false;
        }

        var comparison = caseSens ? StringComparison.Ordinal : StringComparison.OrdinalIgnoreCase;
        return value.Equals(expectedValue, comparison);
    }
    ...
    private async void ConnectProxyAsync(IProxy proxy)
    {
        try {
            ...
            await NetworkStream.WriteAsync(proxyRequest, 0, proxyRequest.Length).ConfigureAwait(false);
    ...
    private void OpenConnectionToRemote()
    {
        ...
        } else if (_client is { Connected: false }) {
            try {
    ...
    private async Task PerformWrite()
    {
        var original = _readWriteCancellationTokenSource;
        if (original == null) {
            return;
    ...
    private unsafe void ReceivedHttpResponse(HttpMessageParser parser)
    {
        ...
        if (parser.Headers.TryGetValue("Set-Cookie", out var cookiesString)) {
        ...
        _c4Queue.DispatchAsync(() =>
        {
            ...
            var dict = parser.Headers.ToDictionary(x => x.Key, object (x) => x.Value);
    ...
    private void ResetConnections()
    {
        _queue.DispatchAsync(() =>
        {
            ...
            var count = 0;
            while (count++ < 5 && _writeQueue is { IsCompleted: false }) {
                Thread.Sleep(500);
            }

            if (_writeQueue is { IsCompleted: false }) {
    ...
    private void SetupAuth()
    {
        ...
        if (auth is { Type: AuthType.HttpBasic }) {
            var username = auth.Username;
            var password = auth.Password;
            var passwordSecure = auth.PasswordSecureString;
            if (username != null && password != null) {
                _logic.Credential = new NetworkCredential(username, password);
            } else if (username != null && passwordSecure != null) {
                _logic.Credential = new NetworkCredential(username, passwordSecure);
            }
        }
    ...
    private void StartInternal()
    {
        ...
        // These should be overwritten.  The user has no business setting them.
    ...
    private bool ValidateServerCert(object sender, X509Certificate? certificate, X509Chain? chain, SslPolicyErrors sslPolicyErrors)
    {
        ...
        if (!onlySelfSigned && sslPolicyErrors != SslPolicyErrors.None) {
            ...
            if (chain.ChainStatus.Any(status => status.Status == X509ChainStatusFlags.PartialChain)) {
                _validationException = new TlsCertificateException("The certificate does not terminate in a trusted root CA.",
...
#endif
                .Any(c => c.Certificate.Equals(_options.PinnedServerCertificate));
    }
    ...
    private async Task WaitForResponse(Stream stream)
    {
        await Task.Factory.StartNew(() => {
            ...
            var responseLength = stream.Read(buffer, 0, buffer.Length);
            var resp = Encoding.UTF8.GetString(buffer, 0, responseLength);
