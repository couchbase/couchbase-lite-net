...
public class DatabaseEncryptionTest(ITestOutputHelper output) : TestCase(output)
{
    private static readonly IDictionary<string, EncryptionKey> KeyCache = new Dictionary<string, EncryptionKey>();
    ...
    public void TestChangeEncryptionKeyNSaveDocOnNewDB()
    {
        ...
        using var saveDoc = db1.GetDefaultCollection().GetDocument("my-doc")?.ToMutable() ?? new MutableDocument("my-doc");
