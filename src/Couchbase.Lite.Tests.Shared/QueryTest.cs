...
public class QueryTest(ITestOutputHelper output) : TestCase(output)
{
    ...
    public void TestMetaRevisionID()
    {
        ...
        using (var q = QueryBuilder.Select(RevID)
            ...
            VerifyQuery(q, (_, row) => {
                row.GetString(0).ShouldBe(doc.RevisionID!);
            });
        ...
        using (var q = QueryBuilder.Select(DocID)
            ...
            VerifyQuery(q, (_, row) => {
                row.GetString(0).ShouldBe(docId);
            });
        }
    ...
    public void TestExpiredNotInQuery()
    {
        ...
        using (var q = QueryBuilder.Select(SelectResult.Expression(Meta.ID))
                   .From(DataSource.Collection(DefaultCollection))) {
            var count = VerifyQuery(q, (_, r) => { r.GetString(0).ShouldBe("doc2"); });
            count.ShouldBe(1);
        }
        ...
        using (var q = QueryBuilder.Select(SelectResult.Expression(Meta.ID))
                   .From(DataSource.Collection(DefaultCollection))
                   .Where(Meta.IsDeleted.EqualTo(Expression.Boolean(true)))) {
            var count = VerifyQuery(q, (_, _) => throw new ShouldAssertException("No results should be present"));
            count.ShouldBe(0);
        }
    }
    ...
    public void TestWhereValued()
    {
        ...
        var tests = new[] {
            ...
            Tuple.Create(name.IsNotValued(), Array.Empty<MutableDocument>()),
            ...
            Tuple.Create(work.IsValued(), Array.Empty<MutableDocument>()),
    ...
    public void TestWhereComparison()
    {
        ...
        LoadNumbers(10);
        RunTestWithNumbers([2, 3, 4, 5, 1, 9], cases);
    }
    ...
    public void TestWhereArithmetic()
    {
        ...
        var m2G8 = new Func<int, int, bool>((x1, _) => x1 * 2 > 8);
        var d2G3 = new Func<int, int, bool>((x1, _) => x1 / 2 > 3);
        var m2E0 = new Func<int, int, bool>((x1, _) => (x1 % 2) == 0);
        var a5G10 = new Func<int, int, bool>((x1, _) => x1 + 5 > 10);
        var s5G0 = new Func<int, int, bool>((x1, _) => x1 - 5 > 0);
        ...
        LoadNumbers(10);
        RunTestWithNumbers([6, 3, 5, 5, 5, 7, 2, 3, 10, 5], cases);
    }
    ...
    public void TestWhereIs()
    {
        ...
        using (var q = QueryBuilder.Select(DocID)
            ...
            numRows.ShouldBe(1, "because one row matches the given query");
        }
    ...
    public void TestWhereMatch()
    {
        ...
        var sSentence = SelectResult.Expression(sentence);
        ...
        using var q = QueryBuilder.Select(DocID, sSentence)
            .From(DataSource.Collection(DefaultCollection))
            .Where(w)
            .OrderBy(o);
        var numRows = VerifyQuery(q, (_, _) =>
    ...
    public void TestParameters()
    {
        ...
        var paramN1 = Expression.Parameter("num1");
        var paramN2 = Expression.Parameter("num2");
    ...
    public void TestMeta()
    {
        ...
        var number1 = Expression.Property("number1");
        var resNumber1 = SelectResult.Expression(number1);

        using var q = QueryBuilder.Select(DocID, Sequence, RevID, resNumber1)
            .From(DataSource.Collection(DefaultCollection))
            .OrderBy(Ordering.Expression(Meta.Sequence));
        var expectedDocIDs = new[] {"doc1", "doc2", "doc3", "doc4", "doc5"};
        var expectedSequences = new[] {1, 2, 3, 4, 5};
        ...
        var numRows = VerifyQuery(q, (n, row) =>
        {
            ...
            seq.ShouldBe(expectedSequences[n - 1]);
            ...
            number.ShouldBe(expectedSequences[n - 1]);
        });
    ...
    public void TestQuantifiedOperatorVariableKeyPath()
    {
        ...
        var varPath = ArrayExpression.Variable("path.city");
        var where = ArrayExpression.Any(ArrayExpression.Variable("path")).In(paths).Satisfies(varPath.EqualTo(Expression.String("San Francisco")));

        using var q = QueryBuilder.Select(sDocId)
            .From(DataSource.Collection(DefaultCollection))
            .Where(where);
    ...
    public void TestGenerateJSONCollation ()
    {
        ...
        ascii.ConvertToJSON().ShouldBeEquivalentToFluent(new object[] { "COLLATE", new Dictionary<string, object>(), (string[])
            [".test"]
        });
    ...
    public void TestAllComparison()
    {
        ...
        var firstResult = new[] { "Aardvark", "Apple", "Zebra", "Ångström", "äpple" };
        var secondResult = new[] { "Aardvark", "Ångström", "Apple", "äpple", "Zebra" };
    ...
    public void TestCountFunctions()
    {
        ...
        var numRows = VerifyQuery(q, (_, row) => { row.GetInt(0).ShouldBe(100); });
        numRows.ShouldBe(1);
    }
    ...
    {
        using (CreateTaskDocument("Task 1", false))
        using (CreateTaskDocument("Task 2", true))
        using (CreateTaskDocument("Task 3", true)) {
...
#pragma warning restore CS8605 // Unboxing a possibly null value.
    ...
    public void TestQueryResultTypes()
    {
        ...
        using (var q = QueryBuilder.Select(SelectResult.Property("array"),
            ...
            VerifyQuery(q, (_, row) =>
            {
                row.GetArray(0).ShouldBeEquivalentToFluent((long[])[1L, 2L, 3L]);
                row.GetArray("array").ShouldBeEquivalentToFluent((long[])[1L, 2L, 3L]);
    ...
    public void TestQueryResultSet()
    {
        ...
        using (var query = QueryBuilder
            ...
            var allRes = resultset.AllResults();
    ...
    public void TestStringToMillis()
    {
        ...
        expectedLocal.AddRange(expectedUTC.Skip(1).Select(entry => entry - offset));
    ...
    public void TestStringToUTC()
    {
        ...
        }).Select(x => DateTimeOffset.Parse(x).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffZ").Replace(".000","")).ToArray();
    ...
    public void TestResultSetAfterDispose()
    {
        ...
        var blob = new Blob("application/octet-stream", [1, 2, 3, 4, 5]);
    ...
    private async Task TestLiveQueryNoUpdateInternal(bool consumeAll)
    {
        ...
        var token = q.AddChangeListener(null, (_, args) =>
        {
            if (consumeAll) {
                var rs = args.Results;
    ...
    private void CreateDocInSeries(int entry, int max)
    {
        var docID = $"doc{entry}";
