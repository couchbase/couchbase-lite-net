...
public sealed class ScopeCollectionTest(ITestOutputHelper output) : TestCase(output)
{
    [Fact]
    ...
    {
        void BadAction() => Db.DeleteCollection(Database.DefaultCollectionName);
    ...
    {
        var col = Db.GetCollection("colA", "scopeB");
    ...
    public void TestDeleteAllCollectionsInScope()
    {
        ...
        collectionsInScopeA.Contains(colA).ShouldBeTrue("Because collection colA is in scopeA");
        collectionsInScopeA.Contains(colB).ShouldBeTrue("Because collection colB is in scopeA");
    ...
    public void TestCollectionNameContainingIllegalChars()
    {
        ...
        /* Create collections with the collection name containing the following characters
           !, @, #, $, ^, &, *, (, ), +, ., :, ;, <, >, ?, [, ], {, }, =, “, ‘, |, \, /,`,~ are prohibited. */
        for (var letter = '!'; letter <= '/'; letter++) {
            if (letter == '%' || letter == '-')
                continue;

            void BadAction() => Db.CreateCollection(str + letter);
    ...
    public void TestCollectionNameLength()
    {
        ...
        void BadAction() => Db.CreateCollection(str);
    ...
    public void TestScopeNameContainingIllegalChars()
    {
        ...
        for (var letter = '!'; letter <= '/'; letter++) {
            ...
            void BadAction() => Db.CreateCollection("abc", str + letter);
    ...
    public void TestScopeNameLength()
    {
        ...
        str += "e";
        str.Length.ShouldBe(252);
        void BadAction() => Db.CreateCollection("abc", str);
    ...
    {
        using var colA = Db.CreateCollection("colA", "scopeA");
        using (Db.CreateCollection("colB", "scopeA")) {
            using (var doc = new MutableDocument("doc"))
            using (var doc1 = new MutableDocument("doc1"))
            using (var doc2 = new MutableDocument("doc2")) {
                doc.SetString("str", "string");
    ...
    public void TestUseCollectionAPIsOnDeletedCollection()
    {
        ...
        Should.Throw<CouchbaseLiteException>(() => colA.AddChangeListener(null, (_, _) => { }),
            "Because AddChangeListener after collection colA is deleted.");

        Should.Throw<CouchbaseLiteException>(() => colA.AddDocumentChangeListener("doc1", (_, _) => { }),
    ...
    public void TestUseCollectionAPIOnDeletedCollectionDeletedFromDifferentDBInstance()
    {
        ...
        using (var otherDB = OpenDB(Db.Name)) {
            ...
            Should.Throw<CouchbaseLiteException>(() => colA1.AddChangeListener(null, (_, _) => { }),
                "Because AddChangeListener after collection colA is deleted from the other db.");

            Should.Throw<CouchbaseLiteException>(() => colA1.AddDocumentChangeListener("doc1", (_, _) => { }),
...
#if !SANITY_ONLY
    [Fact]
    public void TestUseCollectionAPIWhenDatabaseIsClosed() => TestUseCollectionAPI();

    [Fact]
    public void TestUseCollectionAPIWhenDatabaseIsDeleted() => TestUseCollectionAPI();
#endif
    ...
    [Fact]
    public void TestUseScopeAPIAfterDeletingAllCollections()
    {
        //6.5 Get Collections from The Scope Having No Collections
        //GetCollection() NULL
        //GetCollections() empty result
        using (Db.CreateCollection("colA", "scopeA")) {
    ...
    private void TestUseCollectionAPI()
    {
        using var colA = Db.CreateCollection("colA", "scopeA");
        using (var doc = new MutableDocument("doc")) {
            doc.SetString("str", "string");
