...
public sealed class ScopesCollectionsReplicationTest(ITestOutputHelper output) : ReplicatorTestBase(output)
{
    ...
    public void TestAddCollectionsWithCollectionConfig()
    {
        ...
        var colAConfig = config.GetCollectionConfig(colA);
        var colBConfig = config.GetCollectionConfig(colB);
        colAConfig.Equals(colBConfig).ShouldBeFalse("Because the returned configs should be different instances.");
    ...
    [Fact]
    public void TestAddCollectionsFromDifferentDatabaseInstance()
    {
        var colA = Db.CreateCollection("colA", "scopeA");
    ...
    private readonly IList<string> _allowOddIds = ImmutableArray.Create("doc1", "doc3", "doc5", "doc7");
    ...
    public void TestCollectionDocumentIDsPushFilter()
    {
        ...
        using (var colAInDb = Db.GetCollection("colA", "scopeA"))
        using (var colBInDb = Db.GetCollection("colB", "scopeA")) {
            config.GetCollectionConfig(colAInDb).DocumentIDs = _allowOddIds;
            config.GetCollectionConfig(colBInDb).DocumentIDs = _allowOddIds;
        }
    ...
    public void TestCreateReplicatorWithNoCollections()
    {
        ...
        badAction.ShouldThrow<CouchbaseLiteException>("Replicator Configuration must contain at least one collection.");
    }
    ...
    private void ValidatePendingDocumentIds(PendingDocIDSel selection)
    {
        IImmutableSet<string> pendingDocIds;
            ...
            using (var colBInDb = Db.GetCollection("colB", "scopeA")) {
                ...
                } else if (selection == PendingDocIDSel.Filter) {
         ...
         bool docIdIsPending;
            ...
            using (var colBInDb = Db.GetCollection("colB", "scopeA")) {
                ...
                using (var replicator = new Replicator(config)) {
                    ...
                    if (selection == PendingDocIDSel.Create || selection == PendingDocIDSel.Update
                                                                    || selection == PendingDocIDSel.Delete) {
