...
openssl req -x509 -new -nodes -key test-ca.key -sha256 -days 3650 -out test-ca.crt -subj "/CN=Test CA/C=JP/ST=Tokyo/L=Tokyo/O=Couchbase"
openssl req -new -nodes -out test-node.csr -newkey rsa:4096 -keyout test-node.key -subj "/CN=Test Node/C=JP/ST=Tokyo/L=Tokyo/O=Couchbase"
